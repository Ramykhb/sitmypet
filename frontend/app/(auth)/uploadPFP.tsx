...
const UploadPFP = () => {
    ...
    const [imageURL, setImageURL] = useState(`${backendPath}/uploads/pfps/default_pfp.png`);
    ...
    const pickImage = async () => {
        ...
        try {
            ...
            setImageURL(`${backendPath}${res.data.profileImageUrl}`);
        } catch (e: any) {
            if (e.status === 400) {
                ...
                >
                    <View className="flex flex-col flex-1 w-full p-10 items-center">
                        ...
                        <View className={"px-5 w-full mt-10 text-[#0A0A0A]"}>
                            <Text className={"text-4xl text-center"}>No profile is complete without YOU!</Text>
                            <Text className={"text-base text-center text-gray-400 mt-5"}>Add a profile picture to
                                complete your profile</Text>
                            <Text className={"text-base text-center text-gray-400 "}>JPG, JPEG, PNG, PDF, up to
                                5MB</Text>
                        </View>
                        <TouchableOpacity
                            onPress={pickImage}
                            ...
                            {docLoading ? (
                                ...
                                <>
                                    ...
                                    <Text
                                        className="text-[#0A0A0A] text-lg font-bold">{image ? "Change photo" : "Upload PFP"}</Text>
                        ...
                        <Link href={"/(auth)/homeAuth"} className={"mt-3"}>
                            <Text className={"text-[#0A0A0A] text-base underline font-bold"}>Skip this step for
                                now</Text>
